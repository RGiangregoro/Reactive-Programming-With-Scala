...
class BinaryTreeNode(val elem: Int, initiallyRemoved: Boolean) extends Actor {
      ...
      if (e == elem) {
        ...
        getSubtree(e) match {
          case Some(st) =>
            st ! Insert(req, id, e)
          case None =>
            subtrees += (lER(e) -> context.actorOf(BinaryTreeNode.props(e, false)))
            req ! OperationFinished(id)
        }
      }
  ...
  private def lER(e: Int): Position = if (e < elem) Left else Right
  private def getSubtree(e: Int): Option[ActorRef] = subtrees.get(lER(e))
